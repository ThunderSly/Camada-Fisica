...
class RX(object):
    ...
    def __init__(self, fisica):
        ...
        self.size_head = 0
        self.size_real = 0
        self.head_total_pkg = (0).to_bytes(2, byteorder="big")
        self.head_current_pkg = (0).to_bytes(2, byteorder="big")
    ...
    def receiveAnalyze(self, expected):
        self.head_tipo = self.buffer[:1]

        if expected == self.head_tipo:
            return True

        print("Falha na sincronização, esperado: " +
              str(expected) + " Recebido: " + str(self.head_tipo))
        return

    def getNData(self, maxSecs=-1):
...
#        print('leu %s ' + str(temPraLer) )
        ...
        start = time.time()
        now = time.time()
        while (not(self.checkEop) or self.getBufferLen() == 0) and ((now - start < maxSecs) or (maxSecs == -1)):
            now = time.time()
        ...
        self.head_current_pkg = int.from_bytes(self.buffer[0:2], byteorder='big')
        self.head_total_pkg = int.from_bytes(self.buffer[2:4], byteorder='big')
        print(">>>>>>>>>>>>>>  pacote  " + str(self.head_current_pkg) +
              " de " + str(self.head_total_pkg))

        self.head_type = int.from_bytes(self.buffer[4:5], byteorder='big')
        self.size_head = int.from_bytes(self.buffer[5:14], byteorder='big')

        if self.head_type != 4:
            return (self.buffer, self.head_type, self.head_total_pkg, self.head_current_pkg)
        ...
        while True:
            ...
            if c == -1:
                ...
                lista_eops.append(c)
        print(lista_eops)
        lista_eops = lista_eops[:-1]
        if len(lista_eops) > 0:
            for i in lista_eops:
                self.buffer = self.buffer[:i - len(self.byte_stuff)] + self.buffer[i:]
            print(self.buffer)
