...
#        print('leu %s ' + str(temPraLer) )
        ...
        while self.transmissao:
            print(str(self.getBufferLen()))
            if (self.getBufferLen() == self.len_atual) & (self.getBufferLen() != 0):
                self.transmissao = False
            else:
                self.len_atual = self.getBufferLen()
            time.sleep(0.1)
